 ...
 */
function readRoiNamesAndVolumes(rawDict: Record<string, unknown>): Map<number, { name: string; volumeCm3: number | undefined }> {
  const out = new Map<number, { name: string; volumeCm3: number | undefined }>();
  ...
  for (const item of sequence?.Value ?? []) {
    ...
    out.set(roiNumber, { name, volumeCm3: volumeEl?.Value?.[0] });
  }
  return out;
}
...
export function readRTStruct(bytes: ArrayBuffer): ParsedRTStruct {
  ...
  const rawDict = dicomData.dict as Record<string, unknown>;
  const naturalized = DicomMetaDictionary.naturalizeDataset(dicomData.dict) as Record<string, unknown>;
  ...
  const rois: ParsedRoi[] = [...namesAndVolumes.entries()].map(([roiNumber, { name, volumeCm3 }]) => ({
    roiNumber,
    name,
