...
|---|---|---|
| [`rt-geometry-js`](packages/geometry/) | 1.1.0 | The shared core. `GridGeometry` / `Mask3D` / `ScalarField3D`, plane sorting, cross-grid resampling, a histogram/DVH engine, analytic phantoms (cube/sphere/torus with closed-form volumes), comparison metrics (Dice, voxel disagreement, centroid), and mask operations (booleans, mm-radius `dilate`/`erode`, distance transform, `centroid`/`boundingBox`, `crop`/`pad`, connected components). No DICOM. Stability guarantee: [`CONTRACT.md`](packages/geometry/CONTRACT.md). |
...
| [`rtdose-js`](packages/rtdose/) | 0.2.0 | DICOM **RTDOSE** read + dose-volume histograms. D/V/mean/min/max queries against a structure mask, dose sampled at a physical point, cumulative DVH, optional `k³` sub-voxel supersampling for small structures in steep gradients. Every result carries the resampling / interpolation / partial-volume method used to compute it. **Research and QA tooling — not a treatment planning system.** |
